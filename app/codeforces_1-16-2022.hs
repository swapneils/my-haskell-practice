...
replacePairs inp =
  ...
  where
    converter k = shower $ inserter inp k
    ...
    comparing temp acc =
      if (ltemp == el)
        then temp
        else show (max (intread acc) (intread temp))
      where
        ltemp = length temp

-- Taken from divanik's solution
replacePairs' inp@(x : xs) =
  let sums = zipWith (+) xs inp
   in let t = lastOcc sums (>= 10)
       in if t >= 0
            then take t inp ++ [sums !! t] ++ drop (t + 2) inp
            else case xs of
              (y : ys) -> (x + y) : ys
              [] -> [0]
  where
