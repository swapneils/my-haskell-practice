...
unjust :: Maybe a -> a
unjust (Just x) = x
unjust Nothing = error "Was given a Nothing instead of a Just"
...
primes = 2:3:filter
    (\n -> not $ any
        (\p -> n `mod` p == 0)
        (takeWhile (\p -> p * p <= n) $ drop 1 primes))
    [5,7..]
...
twinprimes = filter (\(a,b) -> b - a == 2) $ zip primes $ drop 1 primes
...
main = do
    ...
    putStr "Hello, "
    putStrLn caps
    putStr $ map (\t -> snd t) $ runlength caps
    putStrLn ""
